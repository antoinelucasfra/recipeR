...
if (ing_name != "" && !is.na(ing_value) && ing_value > 0) {
  ...
  cat(sprintf(
    "✓ Added custom density: %s = %.2f g/ml\n\n",
    ing_name,
    ing_value
  ))
} else {
...
cat(sprintf(
  "Total densities now: %d (built-in: %d, custom: %d)\n",
  nrow(all_dens),
  nrow(all_dens) - nrow(custom),
  nrow(custom)
))
...
cat(sprintf(
  "✓ Built-in density table: %d ingredients\n",
  nrow(all_dens_final[all_dens_final$source == "builtin", ])
))
cat(sprintf(
  "✓ Custom densities persisted: %d ingredients\n",
