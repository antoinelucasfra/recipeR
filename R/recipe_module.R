...
search_recipes <- function(query = "", source_filter = NULL) {
  ...
  for (id in names(recipes)) {
    ...
    title_match <- if (query_lower != "") {
      grepl(query_lower, tolower(recipe$title))
    } else {
      TRUE
    }
    source_match <- if (!is.null(source_filter)) {
      recipe$source == source_filter
    } else {
      TRUE
    }
...
get_cuisines <- function() {
  ...
  unique(sapply(recipes, function(r) {
    if (!is.null(r$source)) r$source else "Unknown"
  }))
}
...
get_recipe_stats <- function() {
  ...
  list(
    ...
    avg_ingredients = if (length(recipes) > 0) {
      mean(sapply(recipes, function(r) length(r$ingredients)))
    } else {
      0
    },
    avg_steps = if (length(recipes) > 0) {
      mean(sapply(recipes, function(r) length(r$instructions)))
    } else {
      0
    }
  )
...
sort_recipes <- function(recipes = NULL, by = "title", decreasing = FALSE) {
  ...
  sorted_names <- switch(
    by,
    "title" = names(sort(
      sapply(recipes, function(r) r$title),
      decreasing = decreasing
    )),
    "date_added" = names(sort(
      sapply(recipes, function(r) r$date_added),
      decreasing = decreasing
    )),
    "source" = names(sort(
      sapply(recipes, function(r) r$source),
      decreasing = decreasing
    )),
    names(recipes)
  )
